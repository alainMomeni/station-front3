...
import { FiPlusCircle, FiEdit, FiTrash2, FiFilter, FiToggleLeft, FiToggleRight, FiTag, FiArchive, FiSearch, FiAlertCircle, FiX } from 'react-icons/fi';
...
const GerantCataloguePage: React.FC = () => {
  ...
  const handleOpenProduitModal = (produit?: ProduitCatalogueComplet) => {
    ...
    setProduitEnEdition(produit || null);
  ...
  const handleSaveProduit = async (produitData: ProduitCatalogueComplet) => {
    ...
    if (produitEnEdition && produitData.id) {
      ...
      const nouveauProduit = { ...produitData, id: `P-${uuidv4().slice(0,4)}`, categorieNom: categories.find(c=>c.id === produitData.categorieId)?.nom };
  ...
  // MODIFICATION: renderProduitsTable ne rend plus que la table
  ...
  return (
    <DashboardLayout>
      ...
      {/* Section des filtres pour l'onglet produits, séparée */}
      { activeTab === 'produits' && (
      ...
      {/* Conteneur pour le Spinner ou le contenu de l'onglet actif */}
      {isLoading ? (
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
          {activeTab === 'produits' && renderProduitsTable()}
