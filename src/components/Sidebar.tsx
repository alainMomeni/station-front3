...
import { FiX, FiChevronDown, FiChevronRight, FiLogOut } from 'react-icons/fi';
...
const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen, toggleMobileSidebar }) => {
  ...
  const handleLogout = () => {
    logout();
    // La redirection vers /login sera gérée par la route protection
  };
    ...
    <div className="flex flex-col h-full bg-white shadow-lg">
      ...
      <div className="flex-shrink-0 border-t border-gray-200 p-4">
        <button
          ...
          className="w-full flex items-center px-3 py-2 text-sm text-gray-600 hover:bg-red-50 hover:text-red-700 rounded-md transition duration-150 ease-in-out group"
        >
  ...
  return (
    <>
      {isMobileOpen && ( 
        <div className="fixed inset-0 z-30 bg-black bg-opacity-50 transition-opacity duration-300 md:hidden" onClick={toggleMobileSidebar} aria-hidden="true" /> 
      )}
